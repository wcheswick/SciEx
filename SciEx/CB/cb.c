...
static int deficiency;
double inflection;

static double rgb2lms[9];
static double lms2rgb[9];
static double gammaRGB[3];

colorDeficits deficits[] = {
...
to_colorblind(int r, int g, int b) {
    assert(colorLookup);
    return colorLookup[r*(Z+1)*(Z+1) + g*(Z+1) + b];
}
...
Pixel
livePixelToColorBlind (const Pixel *p) {
    return to_colorblind(p->r, p->g, p->b);
}

Pixel
to_colorblind_raw(int r, int g, int b) {
    double              tmp;
    ...
    p = PIXEL(CLIP(red*Z), CLIP(green*Z), CLIP(blue*Z));
    return p;
}
...
init_deficiency(void) {
    ...
    switch (deficiency) {
        ...
        default:
            assert(0); // "*** inconcievable, unknown deficiency: %d", deficiency);
    }
}
...
init_colorblind(ColorblindDeficiency def) {
    ...
    colorblind_init();
    init_deficiency();
    colorLookup = (Pixel *)malloc(256*256*256 * sizeof(Pixel));
    assert(colorLookup);
    for (int r=0; r<=Z; r++)
        for (int g=0; g<=Z; g++)
            for (int b=0; b<=Z; b++)
                colorLookup[r*(Z+1)*(Z+1) + g*(Z+1) + b] = to_colorblind_raw(r,g,b);
}
